...
- (void)addObjectsFromArray:(NSArray *)otherArray {
    ...
    SEMA_W(_sema_observer)
    for (CDDMutableArrayObserverProxy* ob in self.observers) {
        if ([ob.realObserver respondsToSelector:@selector(didAddFromArray:withArr:)]) {
            [ob.realObserver didAddFromArray:otherArray withArr:self];
        }
    }
    SEMA_S(_sema_observer)
}
...
- (void)removeAllObjects {
    ...
    SEMA_W(_sema_observer)
    for (CDDMutableArrayObserverProxy* ob in self.observers) {
        if ([ob.realObserver respondsToSelector:@selector(didRemoveAllDataWithArr:)]) {
            [ob.realObserver didRemoveAllDataWithArr:self];
        }
    }
    SEMA_S(_sema_observer)
}
