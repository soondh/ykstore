...
- (void)layoutSubviews {
    ...
    self.lineView.frame = CGRectMake(0, self.frame.size.height - bottom_line_height, self.frame.size.width, bottom_line_height);
...
- (void)calculateItemFrames {
    ...
    for (int i = 0; i < self.titlesCount; i++) {
        ...
        CGRect frame = CGRectMake(contentWidth, 0, itemW, self.frame.size.height - bottom_line_height);
        // 记录frame
...
- (void)addProgressViewWithFrame:(CGRect)frame isTriangle:(BOOL)isTriangle hasBorder:(BOOL)hasBorder hollow:(BOOL)isHollow cornerRadius:(CGFloat)cornerRadius {
    ...
    pView.color = self.progressColor.CGColor;
