...
-(void)setTitleArry:(NSArray *)titleArry {
    ...
    [UIView animateWithDuration:0.15 animations:^{
        self.bottomLine.frame = CGRectMake((self.btnWidth - self.bottomLineWidth)/2, self.menuHeight - BottomLineHeight, self.bottomLineWidth, BottomLineHeight);
    }];
...
- (void)updateTitleArray:(NSArray *)titles {
    if (titles == nil || _btnArrys == nil || _btnArrys.count != titles.count) {
        return;
    }
    for (int i=0; i<_btnArrys.count; i++) {
        UIButton *btn = _btnArrys[i];
...
-(void)btnTitleClick:(UIButton *)sender {
    ...
    [UIView animateWithDuration:0.15 animations:^{
        ...
        self.bottomLine.frame = CGRectMake(sender.frame.origin.x + (sender.frame.size.width - self.bottomLineWidth)/2, self.menuHeight - BottomLineHeight, self.bottomLineWidth, BottomLineHeight);
    }];
...
- (void)scrollMenuViewSelectedoffsetX:(NSInteger)selectIndex withOffsetType:(BOOL)types {
    ...
    [UIView animateWithDuration:0.25 animations:^{
        [self.BackScrollView setContentOffset:CGPointMake(offsetX, 0) animated:YES];
        if (types) {
            self.bottomLine.frame = CGRectMake(selectIndex * self.btnWidth + (self.btnWidth - self.bottomLineWidth)/2, self.menuHeight - BottomLineHeight, self.bottomLineWidth, BottomLineHeight);
        }
    }];
