...
- (NSString *)trimmingNewlines {
    return [self stringByReplacingOccurrencesOfString:@"\n" withString:@" "];
}

- (NSString *)trimmingMulNewlines {
    
    if (self && self.length > 2) {
        return [self filterBlankAndBlankLines:self];
    }
    return self;
}
- (NSString *)filterBlankAndBlankLines:(NSString *)str
{
    ...
    for (int i = 0; i < arr.count; i++) {
        ...
        tempStr = [tempStr stringByTrimmingCharactersInSet:[NSCharacterSet newlineCharacterSet]]; //去除掉首尾的空白字符和换行字符
        if (tempStr.length != 0 && ![tempStr isEqualToString:@"\n"]) {
