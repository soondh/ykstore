...
//
//        [[HTTPClient sharedHTTPClient].requestSerializer setValue:[[[UserDataHandler alloc]init] getUserInfo].token  forHTTPHeaderField:@"sanguosha-token"];
//
//
...
//                NSDictionary *userInfo =[NSDictionary dictionaryWithObjectsAndKeys:baseModel.msg, NSLocalizedDescriptionKey,nil];
//                if (baseModel.cd != 0) {
//                    if (baseModel.cd == 16) {
//                        [[IMUtility shareInstance] tokenInvalidate ];
//                        requestBlock(baseModel.cd,baseModel.data,[NSError errorWithDomain:@"" code:baseModel.cd userInfo:userInfo]);
//
//                    } else {
//                        requestBlock(baseModel.cd,baseModel.data,[NSError errorWithDomain:@"" code:baseModel.cd userInfo:userInfo]);
//
...
//
//                } else {
//                    requestBlock(baseModel.cd,baseModel.data,nil);
//
//                }
//            }
//
...
//            if (requestBlock) {
//                requestBlock(1,nil,error);
//            }
//        }];
//    }
//    else
//    {
//        [STTextHudTool showErrorText:@"亲，您的网络不给力"];
//        if (requestBlock) {
//
//            requestBlock(1,nil,nil);
//        }
//
//    }
//}
...
+(void)GET:(NSString *)URLString  parameters:( id)parameters result:(ResultBlock)requestBlock{
    if ([HTTPClient sharedHTTPClient].isReachable) {//有网络
        //[[HTTPClient sharedHTTPClient].requestSerializer setValue:not_null_string([[[UserDataHandler alloc]init] getUserInfo].token)  forHTTPHeaderField:@"sanguosha-token"];
        [[HTTPClient sharedHTTPClient].requestSerializer setValue:[UIApplication sharedApplication].appVersion  forHTTPHeaderField:@"APP-VERSION"];
        [[HTTPClient sharedHTTPClient].requestSerializer setValue:@"IOS" forHTTPHeaderField:@"APP-SYSTEM"];
        [[HTTPClient sharedHTTPClient].requestSerializer setValue:@"application/json"  forHTTPHeaderField:@"Content-Type"];
...
//                    [[IMUtility shareInstance] tokenInvalidate];
//                }
//                requestBlock(baseModel.cd,[baseModel.data jsonBase64Value],nil);
//            }
//
//        } failure:^(NSURLSessionDataTask * _Nullable task, NSError * _Nonnull error) {
//            if (requestBlock) {
...
//                    [[IMUtility shareInstance] tokenInvalidate ];
//                }
//                cacheBlock(baseModel.cd,[baseModel.data jsonBase64Value],nil);
//
//            }else{
...
+ (void)showStrMessage:(NSString *)errMsg {
    [TSMessage showNotificationInViewController:[UIApplication sharedApplication].getCurrentViewConttoller
                                          title:nil
                                       subtitle:errMsg
                                           type:TSMessageNotificationTypeWarning];
...
+(void)POSTWithFormDataURL:(NSString *)URLString parameters:(id)parameters mediaData:(NSMutableArray *)mediaDatas
    ...
    if ([HTTPClient sharedHTTPClient].isReachable) {//有网络
        [HTTPClient sharedHTTPClient].requestSerializer.timeoutInterval         = 30.0;
            ...
            if (mediaDatas.count > 0) {
                for(NSInteger i=0; i<mediaDatas.count; i++) {
                    ...
                    if ([firstObj isKindOfClass:[UIImage class]]) { // 图片
                        ...
                        [formData appendPartWithFileData:eachImgData name:@"file[]" fileName:[NSString stringWithFormat:@"img%d_%ld.jpg", (int)i+1,(NSInteger)([NSDate date].timeIntervalSince1970 * 1000)] mimeType:@"image/jpeg"];
                    }else if ([firstObj isKindOfClass:[NSString class]])
                    {
                        ...
                        [formData appendPartWithFileData:mediaData name:@"file[]" fileName:[NSString stringWithFormat:@"%d_%ld.mp3", (int)i+1,(NSInteger)([NSDate date].timeIntervalSince1970 * 1000)] mimeType:@"audio/mpeg3"];
                    }
                }
            }
...
//+(void)POSTVideoWithFormDataURL:(NSString *)URLString parameters:(id)parameters mediaData:(NSMutableArray *)mediaDatas
//           completionBlock:(ResultBlock)requestBlock{
//    if ([HTTPClient sharedHTTPClient].isReachable) {//有网络
//        [HTTPClient sharedHTTPClient].requestSerializer.timeoutInterval         = 30.0;
//        [[HTTPClient sharedHTTPClient] POST:URLString parameters:parameters constructingBodyWithBlock:^(id<AFMultipartFormData>  _Nonnull formData) {
...
//                        //                    [formData appendPartWithFileURL:mediaUrl name:@"file" error:nil];
//                        NSData *mediaData=[[NSData alloc]initWithContentsOfFile:mediaUrl.absoluteString];
//                        [formData appendPartWithFileData:mediaData name:@"file[]" fileName:[NSString stringWithFormat:@"%d_%ld.mp4", (int)i+1,(NSInteger)([NSDate date].timeIntervalSince1970 * 1000)] mimeType:@"video/mp4"];
//                    }
//                }
//            }
//
